...
function extractRoyalSundaramMotor(text, _sourceFile = "") {
  ...
  const policyStartDate = matchGroup(text, /From\s*00:00(?::00)?\s*hours\s*on\s*(\d{1,2}\/\d{1,2}\/\d{4})/i);
  ...
  const vehicleModel = cleanHdfcValue(matchGroup(text, /Model Description\s*:?\s*([^\n]+)/i));
  const vehicleMake = cleanHdfcValue(
    matchGroup(text, /Make of the Vehicle\s*:\s*([^\n]+?)(?=\s*Model Description|$)/i) ||
      matchGroup(text, /Make of the Vehicle\s*([^\n]+?)(?=\s*Year of Manufacture|\n|$)/i),
  );
  const makeModel = [vehicleMake, vehicleModel].filter(Boolean).join(" ") || vehicleModel;
  const grossVehicleWeight = normalizeAmount(
  ...
  const sgst = normalizeAmount(
    matchGroup(text, /ADD:\s*SGST\s*([0-9,]+\.\d{2})/i) ||
      matchGroup(text, /ADD:SGST\s*([0-9,]+\.\d{2})/i),
  );
  const cgst = normalizeAmount(
    matchGroup(text, /ADD:\s*CGST\s*([0-9,]+\.\d{2})/i) ||
      matchGroup(text, /ADD:CGST\s*([0-9,]+\.\d{2})/i),
  );
