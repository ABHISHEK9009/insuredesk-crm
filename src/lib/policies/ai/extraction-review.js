...
export async function reviewPolicyExtractionWithAi({
  rawText = "",
  extractedData = {},
  sourceFile = "",
} = {}) {
  ...
  try {
    const aiPatch = await requestAiExtractionReview({
      rawText,
      extractedData,
      sourceFile,
      issues,
...
export function normalizePassiveAiPatch(aiPatch = {}, sourceText = "") {
  ...
  for (const [field, entry] of Object.entries(
    aiPatch?.suggestedCorrections || aiPatch?.correctedFields || {},
  )) {
...
async function requestAiExtractionReview({ rawText, extractedData, sourceFile, issues, aiProvider }) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `${aiProvider.provider} review failed with status ${response.status}${detail ? `: ${detail.slice(0, 240)}` : ""}`,
    );
  }
...
async function requestPassiveAiExtractionReview({
  ...
  if (!response.ok) {
    const detail = await response.text().catch(() => "");
    throw new Error(
      `${aiProvider.provider} passive review failed with status ${response.status}${detail ? `: ${detail.slice(0, 240)}` : ""}`,
    );
  }
...
function hasAiSourceEvidence(field, value, sourceText, rawText, fields = {}) {
  const snippet = String(sourceText || "")
    .replace(/\s+/g, " ")
    .trim();
...
function shouldAcceptAiField(field, existingValue, nextValue, data = {}, rawText = "", correction = {}) {
  ...
  if (normalizeComparableFieldValue(field, current) === normalizeComparableFieldValue(field, nextValue))
    return false;
  ...
  if (field === "engineNumber") {
    ...
    return (
      /^(?:ENGINE|MOTOR|SEATING|CHASSIS)$/i.test(current) || (registration && engine.includes(registration))
    );
  }
...
function isInvalidExtractedValue(field, value, data = {}) {
  ...
  if (IDENTIFIER_FIELDS.has(field)) {
    ...
    if (
      (field === "engineNumber" || field === "chassisNumber") &&
      compactIdentifier(data.registrationNumber || data.vehicleNumber) === compact
    )
      return true;
  ...
  if (
    (field === "cubicCapacity" || field === "seatingCapacity" || field === "grossVehicleWeight") &&
    !/\d/.test(text)
  )
    return true;
  return false;
...
function getFieldLabelPatterns(field) {
  ...
  return (
    labels[field] || [
      new RegExp(escapeRegExp(field).replace(/[A-Z]/g, " $&").trim().replace(/\s+/g, "\\s+"), "i"),
    ]
  );
}
...
function hasSourceEvidence(field, value, rawText, fields = {}) {
  ...
  if (field === "fuelType") {
    ...
    return (
      new RegExp(`\\b${escapeRegExp(fuel)}\\b`, "i").test(text) ||
