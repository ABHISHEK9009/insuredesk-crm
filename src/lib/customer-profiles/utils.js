export const CUSTOMER_PROFILE_STATUS_OPTIONS = [
  "New Lead",
  "Follow-up Required",
  "Interested",
  "Not Interested",
...
export const CUSTOMER_PROFILE_FOLLOW_UP_OUTCOMES = [
  "Interested",
  "Call Back Later",
  "Not Interested",
  "Converted",
...
export function sanitizeCustomerProfilePayload(payload = {}) {
  return {
    ...
    selectedLOBs: Array.isArray(payload.selectedLOBs)
      ? payload.selectedLOBs.map((item) => asText(item, 80)).filter(Boolean)
      : [],
    lobDetails:
      payload.lobDetails && typeof payload.lobDetails === "object"
        ? JSON.parse(JSON.stringify(payload.lobDetails))
        : {},
    ...
    followUpOutcome: CUSTOMER_PROFILE_FOLLOW_UP_OUTCOMES.includes(payload.followUpOutcome)
...
export function serializeCustomerProfile(profile) {
  return {
    ...
    email: profile.email || "",
    dob: profile.dob ? (profile.dob instanceof Date ? profile.dob : new Date(profile.dob)).toISOString().split("T")[0] : "",
